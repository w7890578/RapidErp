    ...
    public partial class AddPurchaseStorageOrdersDetail : System.Web.UI.Page
    {
        ...
        private void LoadPage()
        {
            ...
            switch (type)
            {
                case "采购入库":
                    documentName = "采购订单";
                    ControlBindManager.BindListBox("select OrdersNumber,OrdersNumber+' ('+HTNumber+')' as text  from CertificateOrders where OrderStatus ='未完成' and ISNULL (CheckTime ,'') !='' order by CreateTime asc", drpOrdersNumber, "OrdersNumber", "text");
                    trSaleOrder.Visible = false;
                    break;
                case "采购退料出库":
                    documentName = "采购订单";
                    ControlBindManager.BindListBox("select OrdersNumber  from CertificateOrders where OrderStatus ='未完成' and ISNULL (CheckTime ,'') !='' order by CreateTime desc", drpOrdersNumber, "OrdersNumber", "OrdersNumber");
                    break;
                case "盘盈入库":
                    documentName = "盘点编号";
                    ControlBindManager.BindListBox(@"select InventoryNumber 
from StockInventoryLog where ISNULL ( AuditeTime ,'')!=''", drpOrdersNumber, "InventoryNumber", "InventoryNumber");
                    break;
                case "盘亏出库":
                    documentName = "盘点编号";
                    ControlBindManager.BindListBox(@"select InventoryNumber 
...
and OdersType  in ('加急订单','正常订单')", drpOrdersNumber, "OdersNumber", "customerordernumber");
                    break;
                case "包装出库":
                    documentName = "销售订单";
...
and OrderStatus ='未完成' 
and ISNULL (CheckTime ,'') !=''
and OdersType  in ('加急订单','正常订单')", drpOrdersNumber, "OdersNumber", "OdersNumber");
                    break;
                case "维修出库": documentName = "维修订单";
                    ControlBindManager.BindListBox(@"select OdersNumber from SaleOder where ProductType ='贸易' 
and OrderStatus ='未完成' 
and ISNULL (CheckTime ,'') !=''
and OdersType  in ('维修订单')", drpOrdersNumber, "OdersNumber", "OdersNumber");
                    break;
                case "样品出库":
                    documentName = "样品订单";
                    ControlBindManager.BindListBox(@"select OdersNumber from SaleOder where ProductType ='贸易' 
and OrderStatus ='未完成' 
and ISNULL (CheckTime ,'') !=''
and OdersType  in ('样品订单')", drpOrdersNumber, "OdersNumber", "OdersNumber");
                    break;
                case "生产退料入库":
                    documentName = "开工单号";
                    break;
