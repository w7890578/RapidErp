        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (ToolManager.CheckQueryString("IsGenerateGX"))
                {
                    ...
                    Response.Write(WorkOrderManager.GenerateGX(userId));
                    Response.End();
                    return;
                }
                //生成小组开工单
                ...
                {
                    string userId = ToolCode.Tool.GetUser().UserNumber;
                    Response.Write(WorkOrderManager.GenerateXZ(userId));
                    Response.End();
                    return;
                }
                //更新小组开工单班组【制造】
...
,t.库存数量,t.在制品数量,t.需要生产数量,t.交期,t.行号,t.客户产品编号 from ({0})t where 1=1", WorkOrderManager.GetOrderNofinesfinishedDetail(), userId);
            ...
            sqls.Add(sql);
            sql = string.Format(@"insert into T_WorkOrder_Temp(id,Qty,User_id)
