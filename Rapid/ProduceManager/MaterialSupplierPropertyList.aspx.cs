...
{
    public partial class MaterialSupplierPropertyList : System.Web.UI.Page
    {
        ...
        { 
            if (!IsPostBack)
            {
                ...
                {
                    string temp = MaterialSupplierPropertyManager.DeleteData(ToolManager.GetQueryString("ids"));
                    string id = ToolManager.GetQueryString("Id");
                    bool restult = temp == "1" ? true : false;
                    if (restult)
                    {
                        Tool.WriteLog(Tool.LogType.Operating, "删除原材料供应商信息"+ToolManager.ReplaceSingleQuotesToBlank(id), "删除成功");
                        Response.Write(temp);
                        Response.End();
                        return;
                    }
                    ...
                    {
                        Tool.WriteLog(Tool.LogType.Operating, "删除原材料供应商信息" + ToolManager.ReplaceSingleQuotesToBlank(id), "删除失败！原因" + temp);
                        Response.Write(temp);
                        Response.End();
                        return;
                    }
                ...
                {
                   GetPageMaterial("AddOrEditMaterialSupplierProperty.aspx", "btnSearch", "350", "600");
                }
                
            } 
        }
        /// <summary>
        /// 重写GetPage(原材料信息表，，联合主键)
        /// </summary>
        /// <param name="url"></param>
        /// <param name="btnId"></param>
        /// <param name="height"></param>
        /// <param name="width"></param>
        public  void GetPageMaterial(string url, string btnId, string height, string width)
        {
            hasEdit = ToolCode.Tool.GetUserMenuFuncStr("L0112", "SuppliersEdit");
            int pageCount = 0;//总页数 
...
<a href='###'  value='{0}' onclick=""OpenDialog('{2}?MaterialNumber={0}&SupplierId={7}&date={3}','{4}','{5}','{6}')""> <img src='../Img/037.gif' width='9' height='9' />
<span > [</span>   <label class='edit'>编辑</label> <span >]</span></a>
</span>
</td></tr>", dr[1], tdTextTemp, url, DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"), btnId, height, width, Server.UrlEncode(dr["供应商编号"].ToString()), dr["Guid"],hasEdit);
            }
