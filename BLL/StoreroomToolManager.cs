...
(MaterialNumber ,CustomerMaterialNumber ,SupplierMaterialNumber,
MoveTime ,WarehouseNumber ,OrdersNumber 
,Income  ,Balances ,HandledPerson ,MoveReasons,Remark) 
select mwld.MaterialNumber,
...
mwld.DocumentNumber 
,a.Qty ,
mwld.InventoryQty ,'{0}','{1}',mwld.Remark from MaterialWarehouseLogDetail mwld inner join (
select MaterialNumber,sum(qty) as Qty from MaterialWarehouseLogDetail where WarehouseNumber='{2}'
group by MaterialNumber 
) a on a.MaterialNumber=mwld.MaterialNumber where mwld.WarehouseNumber='{2}'", userName, type, warehouseNumber);
            }
            return sql;
        }
